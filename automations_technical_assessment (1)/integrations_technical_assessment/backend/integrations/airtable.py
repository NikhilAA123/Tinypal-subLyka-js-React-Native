...
This module handles OAuth2 authentication, token management, and data retrieval from Airtable.
Commit Purpose: Implement comprehensive comments for the Airtable backend integration.
"""
...
# Base authorization URL for Airtable OAuth2
...
# Scopes required for Airtable integration
...
async def authorize_airtable(user_id, org_id):
    """
    Generates the Airtable authorization URL and stores state/verifier in Redis.
    """
    state_data = {
    ...
    # Store state and verifier in Redis with a 10-minute expiration
...
async def oauth2callback_airtable(request: Request):
    """
    Handles the OAuth2 callback from Airtable, exchanges the code for a token, and stores it in Redis.
    """
    ...
    # Retrieve saved state and verifier from Redis
...
async def get_airtable_credentials(user_id, org_id):
    """
    Retrieves and deletes Airtable credentials from Redis.
    """
...
def create_integration_item_metadata_object(
    ...
    """
    Converts Airtable API response into a standardized IntegrationItem object.
    """
...
def fetch_items(
    ...
    """
    Recursively fetches bases from Airtable API, handling pagination with offsets.
    """
...
async def get_items_airtable(credentials) -> list[IntegrationItem]:
    """
    Fetches all bases and their respective tables from Airtable.
    """
    credentials = json.loads(credentials)
    ...
    for response in list_of_responses:
        ...
        if tables_response.status_code == 200:
            ...
            for table in tables_response['tables']:
                # Add table metadata linked to the parent base
                list_of_integration_item_metadata.append(
